...
class Dataset_Custom(Dataset):
    def __init__(self, root_path, split='train', size=None,
        ...
        self.set_type = type_map[split] # 数据集的类型
        self.features = features         # 特征类型 M S MS 
        self.target = target            # 'OT'
        self.scale = scale              # 是否归一化，                          
        
        '''
            是否使用时间特征 
                0: 禁用时间特征编码， 模型仅使用原始序列数据，不添加任何时间相关信息 
                1：启用时间特征嵌，自动生成时间相关的周期特征（如小时、星期、月份等）并编码为向量
        '''
        self.timeenc = timeenc
        self.freq = freq                # 采样频率: 'h' '15min' '30min' '1min' 等等 指导特征生成粒度（如freq='h'按小时解析）
        self.time_col_name = time_col_name  # 时间列的名称，默认是 'date'
        self.use_time_features = use_time_features # 是否使用时间特征

        # train test ratio 训练集，测试集，验证集
...
class Dataset_Test(Dataset):
    def __init__(self, root_path, split=None, size=None,
                 features='M', data_path='ETTh1.csv',
                 target='OT', scale=True, timeenc=1, freq='ME',
                 time_col_name='date', use_time_features=False, 
    ...
    def __read_data__(self):
        ...
        tmp_stamp = df_raw[[self.time_col_name]][border1:border2]
        tmp_stamp[self.time_col_name] = pd.to_datetime(tmp_stamp.date)
        # pred_dates = pd.date_range(tmp_stamp.date.values[-1], periods=self.pred_len + 1, freq=self.freq)
